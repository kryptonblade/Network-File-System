...
void func(int sockfd)
{
    ...
    for (;;)
    {
        ...
        else if (strcmp(token, "CREATE") == 0 || strcmp(token, "DELETE") == 0 || strcmp(token, "COPY") == 0)
        {
            A = 0;
        }
        printf("%s\n", buff);
        ...
        {
            struct ip_port I;
            ssize_t bytes_received = recv(sockfd, &I, sizeof(I), 0);
            if (bytes_received == -1)
            {
            ...
            if (I.port == 504)
            {
                printf("Error Code 504:the source not found!!\n");
                continue;
            }
            // printf("received");
            printf("ip: %sport :%d", I.ip, I.port);
        ...
        else
        {
            ...
            else
            {
                ...
                if (k == 404)
                {
                    printf("Error Code 404:source not found!!\n");
                }
                else if(k == 405)
                {
                    printf("Error Code 405:destination not found!!\n");
                }
                else
                {
...
void func1(int sockfd, char *buff)
{
    ...
    for (;;)
    {
        ...
        {
            printf("Error Code 404:file cannot be opened!!\n");
            break;
        }
    }
    printf("\n");
...
void func3(int sockfd, char *buff)
{
    ...
    if (fi.size == 404 && fi.permissions == 404)
    {
        printf("Error Code 404:error getting file information!!\n");
        return;
    }
    printf("file size: %ld\n", fi.size);
